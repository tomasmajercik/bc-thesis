...
if __name__ == "__main__":
    ...
    frames_dir  = Path("../raw/sherbrooke/frames")
    db_path     = Path("../raw/sherbrooke/labels/sherbrooke_gt_pedestrians.sqlite")
    mask_path   = Path("../raw/sherbrooke/masks/obstacle_mask.png")
    ...
    for frame_id in tqdm(frame_ids, desc="Processing frames"):
        ...
        for pid in future_heatmaps.keys():
            save_file = Path(f"../processed/sherbrooke/input/{iterator:04d}.npy")
            gt_dir    = Path("../processed/sherbrooke/target"); gt_dir.mkdir(parents=True, exist_ok=True)
            ...
            coords_dir = Path("../processed/sherbrooke/target_coords")
    ...
    obstacle_mask_file = Path("../processed/sherbrooke/obstacle_mask.npy")
...
# run from `cd data/scripts` with `python sherbrooke_process.py`