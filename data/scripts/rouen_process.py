""" This script processes the Rouen dataset. 
run from `cd data/scripts` with `python rouen_process.py`, make sure the raw data 
...
def rasterize_past_traj(
    ...
    if traj_method == "linear":
        ...
        past = [(f, x, y) for (f, x, y) in traj if frame_id - past_steps <= f < frame_id]
...
if __name__ == "__main__":
    INPUT_CFG, GT_CFG = load_params("configs/params-rouen.yaml")

    save_name   = GT_CFG.get("save_as", "Rouen")
    frames_dir  = Path("../raw/rouen/frames")
    db_path     = Path("../raw/rouen/labels/rouen_gt_pedestrians.sqlite")
    mask_path   = Path("../raw/rouen/masks/obstacle_mask.png")
    ...
    frame_ids = sorted([
        ...
        # for p in frames_dir.glob("00000365.jpg") # DEBUG
    ])
...
# run from `cd data/scripts` with `python rouen_process.py`