...
## MOT16 specific functions
# gt.txt columns: frame_id, pid, x_top_left, y_top_left, width, height, conf, class, visibility
# keep: conf == 1 AND class == 1 (pedestrian)

def load_past_traj(gt_path):
    trajectories = defaultdict(list)

    with open(gt_path) as f:
        for row in csv.reader(f):
            frame_id, pid, x, y, w, h, conf, cls, *_ = row
            if int(conf) != 1 or int(cls) != 1:
                continue
            x, y, w, h = float(x), float(y), float(w), float(h)
            xc     = x + w / 2.0
            yc     = y + h / 2.0
            y_foot = yc + h / 2.0  # == y + h (bottom center)
            trajectories[int(pid)].append((int(frame_id), xc, y_foot))
...
def get_people_in_frame(gt_path, frame_id):
    people = []
    with open(gt_path) as f:
        for row in csv.reader(f):
            fid, pid, x, y, w, h, conf, cls, *_ = row
            if int(fid) == frame_id and int(conf) == 1 and int(cls) == 1:
                people.append(int(pid))
    return people

def get_bbox(gt_path, frame_id, pid):
    with open(gt_path) as f:
        for row in csv.reader(f):
            fid, oid, x, y, w, h, conf, cls, *_ = row
            if int(fid) == frame_id and int(oid) == pid and int(conf) == 1 and int(cls) == 1:
                x, y, w, h = float(x), float(y), float(w), float(h)
                xc = x + w / 2.0
                yc = y + h / 2.0
                return xc, yc, w, h
...
## ---- everything below is identical to the other process scripts ----
...
if __name__ == "__main__":
    ...
    for frame_id in tqdm(frame_ids, desc="Processing frames"):
        # --- load frame ---
        img = cv2.imread(str(frames_dir / f"{frame_id:06d}.jpg"))
        H, W, _ = img.shape # type: ignore

        # --- load annotations ---
        people        = get_people_in_frame(gt_path, frame_id)
        ...
        for pid in people:
            ...
            bbox = get_bbox(gt_path, frame_id, pid)
            if bbox is None:
...
# run from `cd data/scripts` with `python mots16_02_process.py`