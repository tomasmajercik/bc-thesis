"""
Modulated base model for ablation study without past trajectory encoder.
"""
import torch
...
class MultiEncoderUNet(nn.Module):
    def __init__(
        ...
        if use_motion:
            ...
            self.film = FiLM(
                ...
                encoder_channels=[
                    [_w(32,width), _w(64,width), _w(64,width)],   # f1
                    [_w(64,width), _w(128,width),_w(128,width)],  # f2
                    [_w(128,width),_w(256,width),_w(256,width)],  # f3
                ]
            )

        # ---------- Fusion ----------
        # 3 encoders: [impass, ctx, zoom]
        ...
        fused_channels = [
            _w(32,  width) + _w(64,  width) + _w(64,  width),  # f1: 160 @ base
            _w(64,  width) + _w(128, width) + _w(128, width),  # f2: 320 @ base
            _w(128, width) + _w(256, width) + _w(256, width),  # f3: 640 @ base
    ...
    def forward(self, imp, ctx, zoom, past_coords=None, return_attention=False):
        e1 = self.impass_enc(imp)
        e2 = self.ctx_enc(ctx)
        ...
        if self.use_motion and past_coords is not None:
            ...
            e2 = (modulated[0][1], modulated[1][1], modulated[2][1], e2[3])
