"""
Modulated base model for ablation study without obstacle encoder.
"""
import torch
    ...
    def __init__(
        ...
        if use_motion:
            ...
            self.film = FiLM(
                ...
                encoder_channels=[
                    [_w(64,width),  _w(64,width),  _w(64,width)],   # f1
                    [_w(128,width), _w(128,width), _w(128,width)],  # f2
                    [_w(256,width), _w(256,width), _w(256,width)],  # f3
                ]
            )

        # ---------- Fusion ----------
        # 3 encoders: [past, ctx, zoom]
        ...
        fused_channels = [
            _w(64,  width) * 3,   # f1: 192 @ base
            _w(128, width) * 3,   # f2: 384 @ base
            _w(256, width) * 3,   # f3: 768 @ base
            _w(512, width) * 2,   # f4: 1024 @ base
        ]

        # ---------- Decoder ----------
        self.decoder = Decoder(fused_channels)

    def forward(self, past, ctx, zoom, past_coords=None, return_attention=False):
